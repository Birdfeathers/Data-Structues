...
int main()
{
  ...
  while(response != "=")
  {
    ...
    */
    ...
    if(isNumber(response)) calc.push(stof(response));
    else if (response != "=") calc.compute(response);

  }

  /* TODO - If the stack is empty then print "No operands: Nothing to evaluate" */
    if(calc.isEmpty()) cout << "No operands: Nothing to evaluate" << endl;

  /* TODO - Validate the expression
      1. If valid then print the result
      2. Else, print "Invalid expression"*/
      else if(calc.getStackHead() -> next == NULL) cout << calc.getStackHead() -> number << endl;
